...
Array.from(moreImages).forEach((el) => {
	el.onclick = ()=>{
		let imgItem = el.parentNode.getElementsByTagName('img')
	  Array.from(imgItem).forEach((item, index) =>{
	  ...
	  let tmp = modalContent.cloneNode(true)
	  modalContent.parentNode.replaceChild(tmp, modalContent)
	}
})